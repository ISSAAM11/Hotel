...
import {Hotel, NodeMailer, ResetPasswordInit, TwilioMessage} from '../models';
...
export class HotelControllerController {
  ...
  async create(
    ...
    if (await this.userRepository.findOne({ where: { email: hotel.account } }) || await this.hotelRepository.findOne({ where: { account: hotel.account } }) ) {
      ...
      let newUser:User = {
        ...
        tokenRefreshedAt: 0,hotelNumber : 0, myDistributorlist: [], myHotelslist: [],
        getId: function () {
      ...
      await this.hotelRepository.create(hotel).then( res => {
        ...
        console.log(myInformation.hotelNumber)
  ...
  /*  const user = await this.userRepository.findById(idUser)
    ...
    const userhotel = await this.userRepository.findOne({ where: { idHotel: hotel.id } })

    const index = user.myHotelslist.findIndex(idhotel => {
        return idhotel == id
    })
    user.myHotelslist.splice(index, 1,)

    await this.userRepository.replaceById(user.id, user);
    userhotel? await this.userRepository.deleteById(userhotel.id) : null
